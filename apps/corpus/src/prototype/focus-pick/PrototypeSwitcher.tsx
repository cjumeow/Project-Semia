...
import { FOCUS_PICK_VARIANTS, readFocusPickVariantKey } from './focusPickVariants';
...
export function PrototypeSwitcher() {
  const variant = readFocusPickVariantKey();
  const current = FOCUS_PICK_VARIANTS.find((entry) => entry.key === variant)!;
  const index = FOCUS_PICK_VARIANTS.indexOf(current);

  const navigate = useCallback((nextIndex: number) => {
    const next =
      FOCUS_PICK_VARIANTS[
        (nextIndex + FOCUS_PICK_VARIANTS.length) % FOCUS_PICK_VARIANTS.length
      ]!;
    const params = new URLSearchParams(window.location.search);
    params.set('prototype', 'focus-pick');
