...
export function PrototypeSwitcher() {
  const variant = readContextTabsVariantKey();
  const current = CONTEXT_TABS_VARIANTS.find((entry) => entry.key === variant)!;
  const index = CONTEXT_TABS_VARIANTS.indexOf(current);

  const navigate = useCallback((nextIndex: number) => {
    const next =
      CONTEXT_TABS_VARIANTS[
        (nextIndex + CONTEXT_TABS_VARIANTS.length) % CONTEXT_TABS_VARIANTS.length
      ]!;
    const params = new URLSearchParams(window.location.search);
    params.set('prototype', 'context-tabs');
      ...
      <div className="proto-switcher-pill pointer-events-auto max-w-xl rounded-full border border-border bg-surface/95 px-3 py-2 shadow-lg">
        <div className="flex items-center gap-2">
          <button
            ...
            aria-label="Previous context-tabs variant"
          ...
          <button
            type="button"
            className="flex h-8 w-8 items-center justify-center rounded-full text-text-secondary hover:bg-canvas"
            aria-label="Next context-tabs variant"
            onClick={() => navigate(index + 1)}
          >
            →
          </button>
