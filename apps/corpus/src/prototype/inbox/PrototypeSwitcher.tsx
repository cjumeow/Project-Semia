...
export const INBOX_PROTOTYPE_VARIANTS = [
  { key: 'A', label: 'Detail panel' },
  { key: 'B', label: 'Inline actions' },
  { key: 'C', label: 'Flat inbox queue' },
] as const;

export type InboxPrototypeVariantKey =
  (typeof INBOX_PROTOTYPE_VARIANTS)[number]['key'];
...
export function PrototypeSwitcher() {
  ...
  const current = INBOX_PROTOTYPE_VARIANTS.find((item) => item.key === variant)!;
  const index = INBOX_PROTOTYPE_VARIANTS.indexOf(current);

  const navigate = useCallback((nextIndex: number) => {
    const next =
      INBOX_PROTOTYPE_VARIANTS[
        (nextIndex + INBOX_PROTOTYPE_VARIANTS.length) %
          INBOX_PROTOTYPE_VARIANTS.length
      ]!;
    const params = new URLSearchParams(window.location.search);
    params.set('prototype', 'inbox');
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-16 z-[60] flex justify-center px-4">
      <div className="pointer-events-auto flex items-center gap-2 rounded-full border border-border bg-surface px-2 py-1.5 shadow-lg">
        ...
        <p className="min-w-[10rem] text-center font-mono text-[11px] text-text">
